...
def simulate_action_user_list(total_user_size, action_user_size):
    """ generate a user_id list that has buy/sell stock action 
        Args:   <total_user_size:integer>
                <action_user_size:integer>
        Return: action_user_list list
    """     
    action_user_list = random.sample(range(total_user_size), action_user_size)
    return action_user_list

def simulate_buysell_action():
    """ generate a action of buy or sell stock action 
        Return: 0 for buy, 1 for sell
    """     
...
def main():
    ...
    while True:
      ...
      for user_id in action_user_list:
          ...
          user_record = {"uid": user_id, "action": action, "ticker": ticker_name, \
          "qty":stk_qty, "time":time_string}
          producer_user.send(sys.argv[1], user_record) 
      
      for ticker_name in ticker_list:
          user_record = {"uid": action_user_list[0], "action": 0, "ticker": \
          ticker_name, "qty":0, "time":time_string}
          producer_user.send(sys.argv[1], user_record) 
